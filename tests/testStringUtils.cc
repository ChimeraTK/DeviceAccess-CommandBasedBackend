...
BOOST_AUTO_TEST_CASE(binaryStrFromInt_tests) {
  ...
  { // uint32_t
    ...
    for(bool isSigned : {true, false}) {
      ...
      NICE_CHECK_EQUAL(binaryStrFromInt<uint32_t>(0x80000000, 2, isSigned, TRUNCATE).value_or(NO), std::string("\x00\x00", 2));
    }
  }//end uint32_t
  ...
  { // uint64_t
    ...
    for(bool isSigned : {true, false}) {
      ...
      NICE_CHECK_EQUAL(binaryStrFromInt<uint64_t>(0x80000000, 2, isSigned, TRUNCATE).value_or(NO), std::string("\x00\x00", 2));
    }
  }//end uint64_t
...
BOOST_AUTO_TEST_CASE(toUserTypeHexInt_test) {
  ...
  // Boolean
  ChimeraTK::Boolean bAns, bTest;

  bAns = false;
  bTest = ChimeraTK::userTypeToUserType<uint16_t, std::string>("0x");
  BOOST_CHECK_EQUAL(bTest, bAns);

  str = std::string("0000", 4);
  bAns = false;
  bTest = ChimeraTK::userTypeToUserType<uint16_t, std::string>("0x" + str);
  BOOST_CHECK_EQUAL(bTest, bAns);

  str = std::string("0001", 4);
  bAns = true;
  bTest = ChimeraTK::userTypeToUserType<uint16_t, std::string>("0x" + str);
  BOOST_CHECK_EQUAL(bTest, bAns);
  ...
  str = std::string("AB0C", 4); // Case: common value, even number of characters
  uiAns = 0xAB0C;
  uiTest = ChimeraTK::userTypeToUserType<uint16_t, std::string>("0x" + str);
  BOOST_CHECK_EQUAL(uiTest, uiAns);

  str = std::string("A0B", 3); // Case: common value, odd number of characters
  uiAns = 0xA0B;
  uiTest = ChimeraTK::userTypeToUserType<uint16_t, std::string>("0x" + str);
  BOOST_CHECK_EQUAL(uiTest, uiAns);

  str = std::string("0", 1); // Case: zero
  uiAns = 0x0;
  uiTest = ChimeraTK::userTypeToUserType<uint16_t, std::string>("0x" + str);
  BOOST_CHECK_EQUAL(uiTest, uiAns);

  str = std::string("", 0); // Case: trivial
  uiAns = 0x0;
  uiTest = ChimeraTK::userTypeToUserType<uint16_t, std::string>("0x" + str);
  BOOST_CHECK_EQUAL(uiTest, uiAns);
