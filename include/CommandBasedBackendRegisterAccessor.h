 ...
 /** Get a NDRegisterAccessor object from the register name. */
  ...
  class CommandBasedBackendRegisterAccessor : public NDRegisterAccessor<UserType> {
   public:
    ...
    : NDRegisterAccessor<UserType>(registerPathName, flags), 
      ...
      _dev(boost::dynamic_pointer_cast<CommandBasedBackend>( dev)) { //_dev(boost::dynamic_pointer_cast<NumericAddressedBackend>(dev)) {
    ...
    virtual void replaceTransferElement([[maybe_unused]] boost::shared_ptr<TransferElement> newElement) {} //mayReplaceTransferElement = false, so this is just empty.
    ...
    virtual boost::shared_ptr<TransferElement> makeCopyRegisterDecorator() = 0; //look at DOOCS backend to see how it's done. TODO
  ...
  /*--------------------------------------------------------------------------------------------------------------------------------*/
   protected:
    ...
    /** raw accessor */
    //boost::shared_ptr<NumericAddressedLowLevelTransferElement> _rawAccessor; // TODO needed???  // need a mixture of the two: _rawAccessor and _dev.
     ...
     *  must be acceptable to call this function while the device is closed or not functional (see isFunctional()) and
     *  no exception is thrown. */
    virtual void doPreWrite(TransferType, VersionNumber) {} //Martin: Swapps user buffer with the buffer sent to hardware
